...
/** Base class for implementations of the DBSCAN algorithm
  ...
  *
  * @param settings Parameters of the algorithm. See [DbscanSettings]
  *                 for details
  */
...
//  private def this(settings: DbscanSettings, partitioningSettings: PartitioningSettings) =
//    this(settings, partitioningSettings)

//  def this() = this(new DbscanSettings, partitioningSettings)
...
object Dbscan {
  ...
  /** A convenience method which you can use to start clustering
    *
    * @param data A [[org.apache.spark.mllib.clustering.RawDataSet]] which should be clustered
    * @param settings Parameters of the algorithm. See
    *                 [[DbscanSettings]] for details
    * @return A [[DbscanModel]] populated with information about clusters
    */
...
//  private [dbscan]
  def keepOnlyPairsWithKeys [K, V] (pairs: RDD[(K, V)], keysToLeave: RDD[K])
