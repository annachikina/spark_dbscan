package org.apache.spark.mllib.clustering.spatial.rdd

import org.apache.spark.mllib.clustering.DbscanSettings
import org.apache.spark.mllib.clustering.spatial.{Box, PointSortKey}
import org.apache.spark.mllib.clustering.util.PointIndexer
import org.apache.spark.mllib.clustering.spatial.{Box, BoxCalculator, Point, PointSortKey}
...
/** Density-based partitioned RDD where each point is accompanied by its sort key
  ...
  */
//private [dbscan]
class PointsPartitionedByBoxesRDD  (prev: RDD[(PointSortKey, Point)], val boxes: Iterable[Box], val boundingBox: Box)
...
//  private [dbscan]
  def extractPointIdsAndCoordinates (data: RDD[(PointSortKey, Point)]): RDD[(PointId, PointCoordinates)] = {
