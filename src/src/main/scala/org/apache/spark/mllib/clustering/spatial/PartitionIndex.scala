...
import org.apache.spark.mllib.clustering.util.math.DoubleComparisonOperations._
...
/** An indexing data structure which allows fast lookup of point's neighbors
  ...
  */
//private [dbscan]
class PartitionIndex (val partitionBounds: Box,
...
//  private [dbscan]
  val boxesTree = PartitionIndex.buildTree (partitionBounds, partitioningSettings, dbscanSettings)
...
//  private [dbscan]
  def findPotentiallyClosePoints (pt: Point): Iterable[Point] = {
...
//  private [dbscan]
  def findBoxForPoint (pt: Point, root: BoxTreeItemWithPoints): BoxTreeItemWithPoints = {
...
//private [dbscan]
object PartitionIndex extends DistanceCalculation {
