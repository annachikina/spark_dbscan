...
import org.apache.spark.mllib.clustering.spatial.{DistanceCalculation, PointSortKey}
import org.apache.commons.math3.ml.distance
import org.apache.spark.mllib.clustering.spatial._
import org.apache.commons.math3.ml.distance.DistanceMeasure

import scala.collection.mutable.{HashSet, ListBuffer}
import org.apache.spark.rdd.RDD
import org.apache.spark.mllib.clustering.spatial.rdd.{PartitioningSettings, PointsInAdjacentBoxesRDD, PointsPartitionedByBoxesRDD}

import scala.Some
import org.apache.spark.mllib.clustering.util.commandLine.CommonArgs
import org.apache.spark.mllib.clustering.util.debug.DebugHelper
...
/** Implementation of the DBSCAN algorithm which is capable of parallel processing of the input data.
  ...
  *
  * @param settings Parameters of the algorithm. See [[org.apache.spark.mllib.clustering.DbscanSettings]]
  *                 for details
  * @param partitioningSettings Partitioning settings. See [[org.apache.spark.mllib.clustering.spatial.rdd.PartitioningSettings]]
  *                             for details
  */
...
//  private [dbscan]
  implicit val distanceMeasure: distance.DistanceMeasure = settings.distanceMeasure

  /** Runs the clustering algorithm
   *
   * @param data A data set to be clustered. See [[org.apache.spark.mllib.clustering.RawDataSet]] for details
   * @return A [[org.apache.spark.mllib.clustering.DbscanModel]] object which represents clustering results
   */
  override protected def run(data: RawDataSet): DbscanModel = {
    ...
    println("WE ARE INSIDE DISTRIBUTED DBSCAN")
  ...
  /** Finds clusters within one partition
    ...
    *           when this method is called, [[org.apache.spark.mllib.clustering.spatial.Point]] objects in the collection
    *           already contain all necessary metadata, including number of their neighbors
    *
    * @param boundingBox A [[org.apache.spark.mllib.clustering.spatial.Box]] object which represents bounds of the whole dataset
    ...
    */
//  private [dbscan]
  def findClustersInOnePartition (it: Iterator[(PointSortKey, Point)],
  ...
  /** Detects clusters in different partitions which should be merged together and generates mappings
    ...
    */
//  private [dbscan]
  def generateMappings (localData: Seq[Point]): (HashSet[(HashSet[ClusterId], ClusterId)],
