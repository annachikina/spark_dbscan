...
import org.apache.spark.mllib.clustering.spatial.rdd.{BoxPartitioner, PartitioningSettings}
import org.apache.spark.mllib.clustering.{BoxId, DbscanSettings, PairOfAdjacentBoxIds, RawDataSet}
import org.apache.spark.mllib.clustering.spatial.rdd.{BoxPartitioner, PartitioningSettings}
...
/** Calculates box-shaped regions for density-based partitioning (see [[org.apache.spark.mllib.clustering.spatial.rdd.BoxPartitioner]] )
  * and for fast lookup of point's neighbors (see [[org.apache.spark.mllib.clustering.spatial.PartitionIndex]]
  *
  * @param data A raw data set
  */
//private [dbscan]
class BoxCalculator (val data: RawDataSet) {
...
//  private [dbscan]
  def getNumberOfDimensions (ds: RawDataSet): Int = {
    val pt = ds.first()
...
//  private [dbscan]
  def calculateBounds (ds: RawDataSet, dimensions: Int): List[BoundsInOneDimension] = {
...
//  private [dbscan]
  def generateCombinationsOfSplits (splits: List[List[BoundsInOneDimension]],
...
//  private [dbscan]
  def assignAdjacentBoxes (boxesWithEnoughPoints: Iterable[Box]) = {
...
//  private [dbscan]
  def generateDistinctPairsOfAdjacentBoxIds (boxesWithAdjacentBoxes: Iterable[Box]): Iterable[PairOfAdjacentBoxIds] = {
...
//  private [dbscan]
  def shouldAdjacentBoxBeIncludedInPartition (rootBoxId: BoxId, adjacentBoxId: BoxId): Boolean = {
    rootBoxId <= adjacentBoxId
  }

//  private [dbscan]
  def generateEmbracingBox (boxes: Iterable[Box]): Box = {
...
//  private [dbscan]
  def generateEmbracingBoxFromAdjacentBoxes (rootBox: Box): Box = {
