...
import org.apache.spark.mllib.clustering.{BoxId, DbscanSettings}
...
/** Represents a box-shaped region in multi-dimensional space
  ...
  */
//private [dbscan]
class Box (val bounds: Array[BoundsInOneDimension], val boxId: BoxId = 0, val partitionId: Int = -1, var adjacentBoxes: List[Box] = Nil)
...
//  private [dbscan]
  def findLongestDimensionAndItsIndex ()
...
//  private [dbscan]
  def calculateCenter (b: Array[BoundsInOneDimension]): Point = {
