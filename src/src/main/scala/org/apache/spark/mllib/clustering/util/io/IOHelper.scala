package org.apache.spark.mllib.clustering.util.io

import org.apache.spark.mllib.clustering.DbscanModel
import org.apache.spark.SparkContext

import scala.collection.mutable.WrappedArray.ofDouble
import org.apache.spark.mllib.clustering.{ClusterId, DbscanModel, PointCoordinates, RawDataSet}
...
object IOHelper {
  ...
  /** Reads a dataset from a CSV file. That file should contain double values separated by commas
    ...
    * @return A [[org.apache.spark.mllib.clustering.RawDataSet]] populated with points
    */
  ...
  /** Saves clustering result into a CSV file. The resulting file will contain the same data as the input file,
    ...
    * input file
    *
    * @param model A [[org.apache.spark.mllib.clustering.DbscanModel]] obtained from Dbscan.train method
...
//  private [dbscan]
  def saveTriples (data: RDD[(Double, Double, Long)], outputPath: String) {
