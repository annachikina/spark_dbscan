...
class PredictionSuite extends DbscanSuiteBase {
  ...
  test ("DbscanModel should mark all points in RDD") {
    val inputTest = sc.parallelize(Array(create2DPoint (0.5, 0.5),
      create2DPoint (4.5, 0.5)))
    val predictedClusterId = model1.predict(inputTest)

    predictedClusterId.collect()
